  ...
  return (
    <AnimatePresence>
      {showProducts && searchedProducts.length > 0 && (
        <motion.div
          ref={productRef}
          className={`absolute w-full mt-1 bg-white border border-gray-300 rounded shadow-md z-10 overflow-y-auto pb-10 xl:w-[418px] max-h-[450px] xl:mt-[500px] md:mt-[500px] ${
            isHiding ? "pointer-events-none" : ""
          }`}
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: isHiding ? 0 : 1, y: isHiding ? -30 : 0 }}
          exit={{ opacity: 0, y: -20 }}
        ...
        >
          ...
          <div className={`sticky top-0 w-full ${isSticky ? "appColor" : "appColor3"}`}>
            <div className="flex items-center justify-around p-4">
              <MdArrowBackIos onClick={handleClickBack} size={20} className="cursor-pointer" />
          ...
          {searchedProducts.map((product, index) => (
            <div
              ...
              className="cursor-pointer flex xl:gap-8 mt-8 gap-4 text-black bg-white p-2 hover:bg-[#e8e8e8]"
            >
              <div className="w-1/2">
                <img src={product.image} className="xl:w-16 xl:h-16" alt={product.name} />
              </div>
              <div>
                <p>{product.name}</p>
